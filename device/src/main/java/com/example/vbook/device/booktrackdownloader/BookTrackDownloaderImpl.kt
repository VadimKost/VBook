...
class BookTrackDownloaderImpl @Inject constructor(
    ...
    private val receiverState = MutableStateFlow<ResourceState<Long>>(IsConnecting)
    ...
    private val broadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            scope.launch {
                receiverState.value = TransferringData(downloadId)
                deferred.finish()
            }
        }
        ...
        scope.launch {
            receiverState.subscriptionCount.collect {
                if (it > 0 && !bound.value) {
                    onBind()
                    bound.value = true
                    receiverState.value = WaitingForData
                } else if (it == 0) {
                    receiverState.value = IsConnecting
                    bound.value = false
                    onUnbind()
                }
            }
